      ...
      <List sx={{ wordBreak: 'break-word' }}>
        {
          <>
            {renderListItem(
              `You confirm the account that you hold with ${primaryDataRecipient.name} as the location for ${adrDataRecipient.name} to disclose CDR data under CDR Rule 7.5(1)(d).`,
            )}
            {renderListItem(
              `${adrDataRecipient.name} sends your CDR data to your account with ${primaryDataRecipient.name} in accordance with your instruction.`,
            )}
            {renderListItem(
